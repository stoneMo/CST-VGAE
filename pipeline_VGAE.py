...
def train(model, data_loader, optimizer, batch_size, predict_frames, params, 
    ...
    for batch in data_loader['train']:
        ...
        loss_kl = torch.sum(0.5 * (model.mean**2 + torch.exp(model.logvar)- 1. \
                - model.logvar)) /batch_size
...
def pipeline(data_path, model_type, embedd_dim, epoch_num, debug, batch_size, predict_frames, 
    ...
    if pretrain == 1:
        transfer_model('./checkpoints/VGAE/pretrain_model_VGAE_9.22_70.53.pkl', model, device)            
    else:
...
#                 scheduler.step(valid_loss)
                ...
                if valid_loss_mae < best_validation_loss:
                    ...
                    improved_str = '*'
                    # save model 
                    torch.save(model.state_dict(), './checkpoints/VGAE/pretrain_model_VGAE_'+str(int(round(start_time * 1000)))+'.pkl')
                    # save embeddings 
#                     np.save('./checkpoints/VGAE/VGAE_embeddings_data.npy', embeddings)
#                     np.save('./checkpoints/VGAE/VGAE_labels_data.npy', labels)
