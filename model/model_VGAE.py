...
class Model(nn.Module):
    r"""Variational Graph Autoenconder (VGAE).
...
#             st_gcn(128, 128, (1, 3), 1, **kwargs),
    ...
    def encode(self, x):
        ...
        self.logvar = self.fc2(x)
        gaussian_noise = torch.randn(self.mean.size(), device=self.mean.device)
        sampled_z = gaussian_noise*torch.exp(self.logvar.mul(0.5)) + self.mean  #(N, C) --> (N, 32)
    ...
    def forward(self, x, predict_frames):
        ...
        y_land, A_pred = self.decode_land(z)  #(N, 2, -1, 19) 
        land_pred = y_land.permute(0, 2, 3, 1).contiguous()  #(N, -1, 19, 2) 
        pose_result = []
        for i in range(predict_frames):
            y_pose, _ = self.decode_pose(y_land[:,:,i:i+1,:])
